...
export default function NewListingPage() {
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await supabase.from('listings').insert({
      user_id: user.id, title, description, price: parseInt(price), location, category, subcategory, condition, images: imageUrls
    })

    setLoading(false)
    if (error) setMessage('Virhe: ' + error.message)
    else { setMessage('Ilmoitus lisätty!'); setTitle(''); setDescription(''); setPrice(''); setLocation(''); setCategory(''); setSubcategory(''); setCondition(''); setImages([]) }
  }
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '50px auto', padding: '20px' }}>
      ...
      <button onClick={handleSubmit} disabled={loading} style={{ width: '100%', padding: '10px', background: '#333', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
